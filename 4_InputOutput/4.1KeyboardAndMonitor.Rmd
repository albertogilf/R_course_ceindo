...
1. **file**: el nombre del fichero sobre el que se van a leer los datos. Este parámetro es opcional y, en caso de tomar el valor por defecto '""', esperará la introducción de datos por la entrada estándar del programa, que habitualmente es el teclado. En este caso, la entrada de datos terminará cuando se introduzca un retorno de carro. Si se indica un fichero, se puede proporcionar un path relativo al directorio de trabajo actual de R —que podemos consultar con `getwd()`— o un path absoluto.
...
other_variable = scan(what=integer(), n = 1)
```

```{r, eval=FALSE}
print("How many goats got one embryo?")
embryos_number = scan(what = integer(),n = 1, quiet = TRUE)
print("What is the percentage of embryos that were actually born?")
success_embryos_percentage = scan(what = double(), n = 1, quiet = TRUE)
...
Si quiero imprimir valores numéricos, puedo especificar el número de dígitos significativos. Si necesito fijar el número de decimales, es preferible usar funciones como `round()`, `sprintf()` o `formatC()`.

```{r}
myValue = 6/7
print(myValue, digits = 2) # 2 dígitos significativos
print(myValue, digits = 4) # 4 dígitos significativos
```
