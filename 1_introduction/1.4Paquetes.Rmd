...
Podemos obtener ayuda sobre cualquier comando con `?`

```{r}
?data
```

Con el operador `$` podemos acceder a cada una de las variables:
...
```{r}
summary(iris) #Modifica esta línea para cambiar el número de cifras significativas
```

## Cargando un paquete en el entorno
